...
def process_video(video_filename, config=DEFAULT_VIDEO_CONFIG, video_out=None, debug=False, deep_debug=False):

    droplet_info = []

    dish_circle, dish_mask = tools.get_median_dish_from_video(video_filename, config['dish_config'])
    ...
    backsub_learning_rate = config['mog_config']['learning_rate']
    backsub_delay = config['mog_config']['delay_by_n_frame']
